...
export function buildTaskPrompt(spec: AgentSpec): string {
  ...
  const baseFields = ["consent", "state", "age", "household_size", "income_est_annual", "coverage_type"];
...
SUMMARY: After the call, provide a JSON summary with: consent, state, age, household_size, income_est_annual, coverage_type, qualifying_life_event, qualified, disqual_reason, transfer_attempted, transfer_completed.`;
}
